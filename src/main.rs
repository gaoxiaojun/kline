...
use kline::{analyzer::Analyzer, bar::Bar, fractal_detector::FractalDetector, time::timestamp_to_utc, util::{load_bar_from_csv, load_fx_from_csv}};
...
fn main() {
    ...
    println!("Fx Count: {} Parsed_fx count: {}", fxs.len(), parsed_fx.len());
    ...
    for i in 0..fxs.len() {
        let lhs = &fxs[i];
        let rhs = &parsed_fx[i];
        assert!(lhs.time == rhs.time && lhs.fx_mark == rhs.fx_mark && lhs.high == rhs.high && lhs.low == rhs.low);
    }
