...
impl PenDetector {
    ...
    fn state1(&mut self, f: SeqFx) -> Option<SegmentEvent> {
        ...
        if last.is_same_type(&f) {
            ...
            if self.ab_is_pen() {
                // 1.2.2
                self.has_pen = true;
                return Some(SegmentEvent::First(
    ...
    fn state2(&mut self, f: SeqFx) -> Option<SegmentEvent> {
        ...
        if bc_is_pen {
            ...
            if b.is_same_type(&f) {
                // 2.2.1
                let action = pen_rule::merge_same_SeqFx_type(b, &f);
                if action == MergeAction::Replace {
                    // 2.2.1.1
                ...
                let action = pen_rule::merge_same_SeqFx_type(a, &f);
                if action == MergeAction::Replace {
                    // 2.2.2.2
                    self.window.clear();
                    self.window.push(f);
    ...
    fn state3(&mut self, f: SeqFx) -> Option<SegmentEvent> {
        debug_assert!(self.ab_is_pen());
        debug_assert!(self.has_pen);
        debug_assert!(self.window.len() == 2);
        ...
        if bc_is_pen {
            ...
            return Some(SegmentEvent::New(c));
        } else {
            if b.is_same_type(&f) {
                let action = pen_rule::merge_same_SeqFx_type(b, &f);
                if action == MergeAction::Replace {
                    ...
                    self.window.push(f);
                    //self.ab_pen_update();
                    return Some(SegmentEvent::UpdateTo(c));
                }
            } else {
                // 3.2.1
                self.window.push(f);
    ...
    fn state4(&mut self, f: SeqFx) -> Option<SegmentEvent> {
        debug_assert!(self.ab_is_pen());
        debug_assert!(!self
            .window
            .get(-2)
        ...
        if c.is_same_type(&f) {
            // 4.1
            let action = pen_rule::merge_same_SeqFx_type(c, &f);
            if action == MergeAction::Replace {
                // 4.1.1
                self.window.pop_back();
                self.window.push(f);
                if self.bc_is_pen() {
                    // 4.1.1.1
                    self.window.pop_front();
                    return Some(SegmentEvent::New(self.window.get(-1).unwrap().clone()));
                }
            }
        } else {
            // 4.2
    ...
    pub fn on_new_seq_fractal(&mut self, f: SeqFx) -> Option<SegmentEvent> {
